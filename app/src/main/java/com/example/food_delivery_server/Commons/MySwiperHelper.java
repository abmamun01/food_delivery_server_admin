...
import com.example.food_delivery_server.CallBack.MyButtonClickListener;
...
public abstract class MySwiperHelper extends ItemTouchHelper.SimpleCallback {
    ...
    public class MyButton {
        ...
        public boolean onClick(float x, float y) {
            if (clickRegioin != null && clickRegioin.contains(x, y)) {
    ...
    private View.OnTouchListener onTouchListener = (View.OnTouchListener) (view, motionEvent) -> {
        ...
        RecyclerView.ViewHolder swipeHolder = recyclerView.findViewHolderForAdapterPosition(swipePosition);

        View swipedItem = swipeHolder.itemView;
        Rect rect = new Rect();
    ...
    public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        swipePosition=pos;
        if (buttonBuffer.containsKey(swipePosition)) buttonsList=buttonBuffer.get(swipePosition);
        else buttonsList.clear();
        ...
        if (actionState==ItemTouchHelper.ACTION_STATE_SWIPE){
            if (dX<0){
                ...
                if (!buttonBuffer.containsKey(pos)){
                    instantitaMyButton(viewHolder,buffer);
                    buttonBuffer.put(pos,buffer);
                }else {
                    buffer=buttonBuffer.get(pos);
                }
                translationX=dX*buttonBuffer.size()*buttonWidth/itemView.getWidth();
    ...
    public abstract void instantitaMyButton(RecyclerView.ViewHolder viewHolder,List<MyButton> buf);
}
